...
with open("results60.csv", mode='w', newline='') as csvfile:
    ...
    writer.writerow(["ASIN", "Title", "Brand", "Manufacturer", "Sales Rank", "MPN"])
    ...
    for index, asin in enumerate(asins, start=1):
        ...
        if response.status_code == 200:
            ...
            # Fallback check inside AttributeSets
            
            
            # Debugging output to verify the extracted identifiers
            ...
            title, brand, manufacturer, sales_rank, mpn = "Error", "Error", "Error", "Error", "Error"
            
        
        writer.writerow([asin, title, brand, manufacturer, sales_rank, mpn])
