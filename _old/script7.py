...
# Read ASINs from CSV
asins = []
with open("asin_short2.csv", newline='') as csvfile:
...
# Prepare CSV output
with open("results80.csv", mode='w', newline='') as csvfile:
    ...
    for index, asin in enumerate(asins, start=1):
        ...
        if response.status_code == 200:
            ...
            print(f"Error fetching data for ASIN {asin}: {response.status_code} - {response.text}")
