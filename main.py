...
token_expiry_time = time.time() + 1800  # 30 minutes validity
...
with open("bd_asins_leash_collar.csv", newline='') as csvfile:
    ...
    asins = [row[0].strip() for row in reader]

print(f"Processing {len(asins)} ASINs...")

# Prepare CSV output
with open("results_bd.csv", mode='w', newline='') as csvfile:
    ...
    for index, asin in enumerate(asins, start=1):
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 200:
                rate_limit = float(response.headers.get("x-amzn-RateLimit-Limit", 5))  # Default to 5 requests/sec
                success = True
                break
                ...
                time.sleep(1)  # General retry delay
        ...
        time.sleep(1 / rate_limit)  # Respect rate limit dynamically

print("Processing complete. Results saved to results100.csv.")
